...
/mob/living/proc/make_sucking_noise()
	if(gender == FEMALE)
		playsound(src, pick(SEXCON_SOUNDS_MOUTH_SUCK_FEMALE), 25, TRUE, 7, ignore_walls = FALSE)
	else
		playsound(src, pick(SEXCON_SOUNDS_MOUTH_SUCK_MALE), 35, TRUE, 7, ignore_walls = FALSE)
