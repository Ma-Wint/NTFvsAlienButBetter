...
/datum/emote/living/carbon/human/moan/get_sound(mob/living/carbon/human/user)
	if(!user.species)
		return
	if(user.species.sexymoanlights[user.gender])
		return user.species.sexymoanlights[user.gender]
	if(user.species.sexymoanlights[NEUTER])
		return user.species.sexymoanlights[NEUTER]
